...
const CheckoutPage = (props) => {
  ...
  // cookies.set('myBag', JSON.stringify(cart), { path: '/' });
  ...
  const addToCart = (el) => {
    ...
    if (cookie) {
      const cart = objectToArray(cookie);
      cart.push(item);
      cookies.set(cookieName, JSON.stringify(cart), {
        path: '/',
      });
    } else {
      cookies.set(cookieName, JSON.stringify([item]), { path: '/' });
    }
  ...
  const cartItems = cookies.get(cookieName)
    ? objectToArray(cookies.get(cookieName)).map((el) => (
        <div key={el.name}>
